...
import QTipService from "../../services/QTipService";
...
const showQtip = function (target, targetId: string, rolled: number) {
    const qtipOptions = {
        ...
        style: { classes: "qtip-light ddbx-table-roll-tip" }
    };
...
class TableRollService {
    static init() {
        ...
        $("table:not(.ddbx-processed)").each((index, table) => initTable(table, index));
    }
}
...
class ClickableRoller extends Component {
    ...
    render() {
        return (
            <span className="ddbx-table-roll-dice" title="Click to Roll" onClick={this.roll} ref={(el) => { this.rollEl = el; }}>
