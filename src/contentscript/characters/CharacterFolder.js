...
const addCharModalId = (folderId) => `ddbx-add-character-modal-${folderId}`;
const removeCharModalId = (folderId) => `ddbx-remove-character-modal-${folderId}`;
...
class CharacterFolder extends Component {
    ...
    baseCharacterModalContent = (charList: CharacterData[], selectProp: string) => {
        ...
        return (
            <div>
                ...
                <select multiple value={this.state[selectProp]} size={charList.length} className="character-select ddbx-character-multiple-select" onChange={handleSelected}>
    ...
    deleteDialogId = () => {
        return `ddbx-delete-folder-dialog-${this.props.data.id}`;
    }
    ...
    renderUp = () => {
        ...
        return <a className="homebrew-creation-actions-item ddbx-homebrew-character-action" href="javascript:void(0)" onClick={() => this.props.onUp(this.props.data)}>Move Up</a>;
    }

    renderDown = () => {
        if (!this.props.down) return null;
        return <a className="homebrew-creation-actions-item ddbx-homebrew-character-action" href="javascript:void(0)" onClick={() => this.props.onDown(this.props.data)}>Move Down</a>;
    }

    renderRemove = () => {
        if (this.props.characters.length === 0) return null;
        return <a className="homebrew-creation-actions-item ddbx-homebrew-character-action" href="javascript:void(0)" onClick={this.RemoveCharClick}>Remove Characters</a>;
    }

    renderAdd = () => {
        if (this.props.charactersNotOnFolders.length === 0) return null;
        return <a className="homebrew-creation-actions-item ddbx-homebrew-character-action" href="javascript:void(0)" onClick={this.addCharClick}>Add Characters</a>;
    }
            ...
            <div>
                <div className={this.rowClassNames()} onClick={() => this.props.onToggle(this.props.data)} onMouseEnter={this.handleMouseIn} onMouseLeave={this.handleMouseOut}>
                    ...
                    <div className="list-row-col list-row-col-indicator">
                        <div className={`list-row-indicator characters ddbx-character-folder-row-indicator ${this.props.data.expanded ? "open" : "closed"}`} />
                    </div>
                </div>
                <div className="more-info more-info-background" style={{ display: this.props.data.expanded ? "block" : "none" }}>
                    ...
                    <div className="line character marginTop20 marginBottom20 ddbx-character-line" />
